package pe.edu.ulima.torreshanoi;
...
public class TorresHanoi extends Applet implements Runnable {
	...
	private static final int NRO_DISCOS = 3;
	...
	public void run() {
		...
		try {
			while (true) {
				...
				delay(2000);
				synchronized (this) {
					tower = new int[3][NRO_DISCOS];
					for (int i = 0; i < NRO_DISCOS; i++)
						tower[0][i] = NRO_DISCOS - i;
					towerHeight = new int[3];
					towerHeight[0] = NRO_DISCOS;
					if (OSC != null) {
						Graphics g = OSC.getGraphics();
						drawCurrentFrame(g);
						g.dispose();
				...
				//solve(NRO_DISCOS, 0, 1, 2);
				comandos.forEach((String comando) ->{
					System.out.println(comando);
					String[] com = comando.split(",");
					moveOne(Integer.parseInt(com[0]), Integer.parseInt(com[1]));
					delay(3000);
				});
	...
	private void solve(int disks, int from, int to, int spare) {
		...
		if (disks == 1) {
			moveOne(from, to);
			System.out.println("Disco de " + from + " a " + to);
		} else {
	...
	private List<String> readInstructions() throws IOException, URISyntaxException{
		String ruta = getClass().getProtectionDomain().getCodeSource().getLocation().getPath();
		return Files.readAllLines(Paths.get(ruta, 
				"pe/edu/ulima/torreshanoi/movimientos.txt"), Charset.defaultCharset());
